...
export async function getProductSummary(productId) {
  const res = await fetch(`${API_BASE_URL}/api/ai/summary/${productId}`);
  return res.json();
}
...
export async function getDiscountCategories() {
  const res = await fetch(`${API_BASE_URL}/api/ai/discounts`);
  return res.json();
}

export async function getReviewSummary(productId) {
  const res = await fetch(`${API_BASE_URL}/api/ai/review-summary/${productId}`);
  return res.json();
} 